...
def generate_grid():
    ...
    for y in range(GRID_HEIGHT):
        for x in range(GRID_WIDTH):
            if (x, y) != tuple(player_pos) and random.random() < 0.2: grid[y][x] = 1
    return grid
...
def generate_valid_barriers(level, grid):
    ...
    while True:
        ...
        if food_cells.issubset(reachable): return barriers, adjacency, sorted_rows
...
def get_target_for_ghost2(player_pos):
    ...
    if tuple(player_pos) in neighbors:
        ...
        if player_zone is None or player_zone == 0:
            return (tuple(player_pos))
        elif player_zone==ghost_zone: return (tuple(player_pos))
        else:
...
def draw_pacman(pos):
    ...
    if not game_over:
        if mouth_closing:
            mouth_angle -= 1
            if mouth_angle <= 1: mouth_closing = False
        else:
            mouth_angle += 1
            if mouth_angle >= 25: mouth_closing = True
    else:
        mouth_angle = 1
    ...
    angle_offset = 0
    if dx == 1 and dy == 0: angle_offset = 0
    elif dx == 0 and dy == -1: angle_offset = 90
    elif dx == -1 and dy == 0: angle_offset = 180
    elif dx == 0 and dy == 1: angle_offset = 270
    ...
    if dx == 1: eye_offset_x, eye_offset_y = 5, -10
    elif dx == -1: eye_offset_x, eye_offset_y = -5, -10
    elif dy == -1: eye_offset_x, eye_offset_y = -7, -5
    elif dy == 1: eye_offset_x, eye_offset_y = -7, 5
    elif dx == 0 and dy == 0: eye_offset_x, eye_offset_y = 5, -10
...
def draw_enemies():
    ...
    if not enemy_positions: init_enemies_for_level()
...
def draw_info():
    text = font.render(f"Level: {level}/{LEVEL_COUNT} | Difficulty: {get_difficulty(level)} | Points: {score % POINTS_TO_NEXT_LEVEL}/{POINTS_TO_NEXT_LEVEL}", True, WHITE)
...
def draw_game_over():
    if game_won:
        ...
        text = font.render("Гру завершено!", True, (255, 100, 100))
...
def get_target_for_food_hunter(player_pos):
    ...
    if tuple(player_pos) in neighbors:
        ...
        if target_food: return (tuple(target_food))
        else: return (tuple(player_pos))
...
def init_enemies_for_level():
    ...
    enemy_positions = [(0,0), (GRID_WIDTH - 1, 0), (0, GRID_HEIGHT - 1), (GRID_WIDTH - 1, GRID_HEIGHT - 1)]
    enemy_team_mode = False
    if level>5: enemy_team_mode = True
    if enemy_team_mode:
        ...
        if level==1: enemy_roles = ['random', 'random', 'random', 'random']
        elif level==2: enemy_roles = ['bfs', 'random', 'random', 'random']
        elif level==3:
            if random.random() < 0.5: enemy_roles = ['bfs', 'predict', 'random', 'random']
            else: enemy_roles = ['bfs', 'food_hunter', 'random', 'random']
        elif level==4:
            if random.random() < 0.5: enemy_roles = ['bfs', 'predict', 'food_hunter', 'random']
            else: enemy_roles = ['bfs', 'food_hunter', 'predict', 'random']
        else:
            if random.random() < 0.5: enemy_roles = ['bfs', 'predict', 'food_hunter', 'predict']
            else: enemy_roles = ['bfs', 'food_hunter', 'predict', 'food_hunter']
...
while running:
    ...
    if not game_over:
        ...
        if current_time - last_enemy_move_time > ENEMY_MOVE_INTERVAL:
            ...
            if enemy_team_mode:
                ...
                if (team_mode==2):
                    ...
                    if path0 and len(path0) >= 2: new_positions.append(path0[1])
                    else: new_positions.append(enemy_positions[0])
                    i = None
                    for idx in range(len(sorted_rows) - 1):
                        if sorted_rows[idx] <= player_pos[1] < sorted_rows[idx + 1]:
                            ...
                            break
                    if i==None:
                        if player_pos[1] < sorted_rows[0]: i=-1
                        else: i= len(sorted_rows) - 1

                    if i==-1: target_pos_2 = tuple(player_pos)
                    else:
                        ...
                        for x in range(1, GRID_WIDTH - 1):
                            ...
                            if 0 <= above[1] < GRID_HEIGHT:
                                if no_barrier_between(cell, above):
                                    target_pos_2 = cell
                                    break
                    if target_pos_2 is None: target_pos_2 = tuple(player_pos)
                    path1 = bfs(enemy_positions[1], target_pos_2, adjacency)
                    neighbors = adjacency.get(enemy_positions[1], [])
                    if tuple(player_pos) in neighbors:
                        new_positions.append(tuple(player_pos))
                    else:
                        if path1 and len(path1) >= 2 and path1[1] not in new_positions: new_positions.append(path1[1])
                        else: new_positions.append(enemy_positions[1])

                    if i==len(sorted_rows) - 1: target_pos_3 = tuple(player_pos)
                    else:
                        if sorted_rows and len(sorted_rows) >= 2 and i + 1 < len(sorted_rows):
                            target_row_3 = sorted_rows[i + 1] - 1
                        else: target_row_3 = min(GRID_HEIGHT - 2, (sorted_rows[0] + 1) if sorted_rows else GRID_HEIGHT // 2)
                        target_pos_3 = None
                        for x in range(1, GRID_WIDTH - 1):
                            cell = (x, target_row_3)
                            ...
                            if 0 <= below[1] < GRID_HEIGHT:
                                if no_barrier_between(cell, below):
                                    target_pos_3 = cell
                                    break
                    if target_pos_3 is None: target_pos_3 = tuple(player_pos)
                    path2 = bfs(enemy_positions[2], target_pos_3, adjacency)
                    neighbors = adjacency.get(enemy_positions[2], [])
                    if tuple(player_pos) in neighbors:
                        new_positions.append(tuple(player_pos))
                    else:
                        if path2 and len(path2) >= 2 and path2[1] not in new_positions: new_positions.append(path2[1])
                        else: new_positions.append(enemy_positions[2])
                ...
                for i, pos in enumerate(enemy_positions):
                    ...
                    if role == 'bfs':
                        ...
                        if path and len(path) >= 2:
                            next_step = path[1]
                            if next_step in new_positions: new_positions.append(enemy_positions[i])
                            else: new_positions.append(next_step)
                        else: new_positions.append(pos)
                    elif role == 'predict':
                        predicted_pos = (
                            player_pos[0] + 2 * last_player_direction[0],
                            player_pos[1] + 2 * last_player_direction[1]
                        ...
                        if tuple(player_pos) in neighbors:
                            ...
                            if path_to_food and len(path_to_food) > 1:
                                ...
                                if next_step not in new_positions: new_positions.append(next_step)
                                else: new_positions.append(pos)
                            else: new_positions.append(pos)
                        else:
        ...
        for enemy in enemy_positions:
            if tuple(player_pos) == enemy: game_over = True
